...
class Toolbar(QHBoxLayout):
    def __init__(self, editor):
        ...
        self.TOOLS = {
            "generate_button": [
                ...
                ("button_generate_bash", "Generate Bash"),
                self.editor.generate_bash,
            ],
            "save_button": [
                self.save_button,
                ("button_save", "Save"),
                self.editor.save_graph,
            ],
            "load_button": [
                self.load_button,
                ("button_load", "Load"),
            ...
            "run_bash_button": [
                self.run_bash_button,
                ("button_run_bash", "Run Bash Script"),
                self.editor.run_bash,
            ],
            "clipboard_button": [
                self.copy_button,
            ...
            "keyboard_button": [
                ...
                self.editor.open_keyboard_shortcuts,
            ],
            "keyboard_action": [
            ...
            "fullscreen_button": [
                ...
                self.editor.toggle_full_screen,
            ],
            "fullscreen_action": [
    ...
    def copy_to_clipboard(self):
        return lambda: QApplication.clipboard().setText(
            self.editor.output_text.toPlainText()
        )
        ...
        for items in self.TOOLS:
            if isinstance(self.TOOLS[items][0], QToolButton):
                self.TOOLS[items][0].setToolTip(
                    Traduction.get_trad(*self.TOOLS[items][1])
                )
    ...
    def _apply_theme(self):
        for button in self.TOOLS:
            if not isinstance(self.TOOLS[button][0], QToolButton):
                self.TOOLS[button][0].setIcon(
                    Icon.load_icon("menu_app", button.rsplit("_", 1)[0])
                )
